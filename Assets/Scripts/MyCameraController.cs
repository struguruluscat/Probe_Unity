...
public class MyCameraController : MonoBehaviour
{
    ...
    public float myDistance = 12.0f;
    public float myTargetHeight = 5f; 
    ...
    private void CameraMovement() 
    {
        ...
        Quaternion rotation = Quaternion.Euler(positionTarget.rotation.eulerAngles);
    //    Vector3 vTargetOffset;
    //    vTargetOffset = new Vector3(0, positionTarget.position.y - myTargetHeight, 0);
    //    Vector3 position = positionTarget.transform.position - (rotation * Vector3.forward * myDistance + vTargetOffset);
        transform.position = Vector3.Lerp(transform.position, positionTarget.position, step);
    }
    ...
    private void CameraRotation() 
    {
        ...
        Quaternion nextRotation = Quaternion.Slerp(transform.rotation, rotationTarget.rotation, desiredRot);
    ...
    private void CameraTypeChanger() 
    {
        if (Input.GetKeyDown(KeyCode.C))
        {
            if (currentCameraType == "type1")
                currentCameraType = "type2";
            else if (currentCameraType == "type2")
                currentCameraType = "type1";
        }
    }
}